...
void ClientCommand (edict_t *ent)
{
	...
	if (Q_stricmp (cmd, "use") == 0)
		...
		Cmd_Drop_f (ent);
	/*
		added code stuff for non-ammo attacks
	*/
	else if (Q_stricmp (cmd, "MagicFor") == 0)
	{
		...
		if(ent->health > 1)
		{
			...
			T_Damage (ent, ent, ent, dir, ent->s.origin, vec3_origin, 1, 0, 0, MOD_BLASTER);
		}
		else
		{
	...
	else if (Q_stricmp (cmd, "MagRadialS") == 0)
	{
		if(ent->mana >= 5)
		{
			...
			VectorScale (forward, -2, ent->client->kick_origin);
			ent->client->kick_angles[0] = -1;
			Magic_Slow_Radial (ent, start, forward, 200, 1000,200); 
	...
	else if (Q_stricmp (cmd, "MagMixS") == 0)
	{
		if(ent->mana >= 5)
		{
			...
			VectorScale (forward, -2, ent->client->kick_origin);
			ent->client->kick_angles[0] = -1;
			Magic_Slow_Mix (ent, start, forward, 20); 
